...
# Greatly stabilizes and improves the network training procedure
class ReplayMemory:
...
class PolicyNetwork(nn.Module):
    def __init__(self, n_inputs, n_outputs, dropout_prob=0.5):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        return self.fc3(self.dropout(x))  # Returns the Q-values for each state, action
...
def optimize_model(memory, policy, target_net, optimizer):
    ...
    # Compute Q-values of every state for all actions (shape: (batch_size, n_actions))
    # Select the Q-value corresponding to each sample’s taken action, yielding a (BATCH_SIZE) tensor.
    state_action_values = policy(state_batch).gather(1, action_batch).view(-1)
    ...
    next_state_values[non_final_mask] = (
        ...
        .max(1)[0]
        .detach()  #  shape of nonfinal: (n_nonfinal, n_actions) -> (n_nonfinal)
    )  # (batch_size)
...
def main():
    ...
    n_states = env.observation_space.shape[0]  # 4 state values
    n_actions = env.action_space.n  # 2: Left and Right

    policy = PolicyNetwork(n_states, n_actions).to(device)
    target_net = PolicyNetwork(n_states, n_actions).to(device)
    ...
    if SHOULD_TRAIN:
        for i_episode in range(1, NUM_EPISODES + 1):
            ...
            for t in range(1, EPISODE_STEP_LIMIT + 1):  # Limit steps per episode
                log_prob_action, action = select_action(state, policy, n_actions)
